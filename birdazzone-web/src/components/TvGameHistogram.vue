...
const fetchAttempts = async () => {
  ...
  if (response.esit) {
    histogramValues.value = response.data!
    .filter(x => x.absoluteFrequency > 3)
    .map(x => ({
      label: x.value,
...
<div>
    <Histogram 
      v-if="!loading" :chart-title="'Played words with more than 3 tentatives'" :values="histogramValues" :serie-name="SERIE_NAME" />
  </div>
</template>
